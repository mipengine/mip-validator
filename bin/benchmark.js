...
Promise.resolve()
    .then(x => npmBenchmark(false))
    .then(x => npmBenchmark(true))
    .then(x => httpBenchmark(false))
    .then(x => httpBenchmark(true))
...
function npmBenchmark(fast) {
    ...
    bm.begin('NPM', fast);
    var results = htmls.forEach(html => validator.validate(html));
    bm.end('NPM', fast);
...
function socketBenchmark() {
    return new Promise(function(resolve, reject) {
        ...
        client.on('data', function(data) {
            ...
            result = tokens.pop();
            tokens.forEach(token => successCount++);
        ...
        client.on('close', function() {
            console.log('[benchmark] Connection closed');
            reject();
        });
    });
}
